...
export default function DashboardPage() {
  ...
  return <main className="min-h-screen bg-[#f7f7f5] text-[#222]"><div className="mx-auto flex min-h-screen max-w-[1400px]">
    <aside className="hidden w-64 shrink-0 border-r border-black/10 bg-white px-5 py-7 md:block"><div className="flex items-center gap-3"><div className="grid h-9 w-9 place-items-center rounded-xl bg-[#173b32] text-sm font-bold text-white">L</div><span className="text-xl font-semibold">loggin</span></div><nav className="mt-10 space-y-1"><button onClick={() => setView("today")} className={`w-full rounded-lg px-3 py-2 text-left text-sm ${view === "today" ? "bg-[#f2f2ef] font-semibold" : "text-[#777]"}`}>Overview</button><button disabled={!office} onClick={() => setView("people")} className={`w-full rounded-lg px-3 py-2 text-left text-sm ${view === "people" ? "bg-[#f2f2ef] font-semibold" : "text-[#777] disabled:opacity-40"}`}>People</button><button disabled={!office} onClick={() => setView("attendance")} className={`w-full rounded-lg px-3 py-2 text-left text-sm ${view === "attendance" ? "bg-[#f2f2ef] font-semibold" : "text-[#777] disabled:opacity-40"}`}>Attendance</button></nav><div className="mt-9"><div className="mb-2 flex items-center justify-between px-3"><p className="text-[11px] font-semibold uppercase tracking-widest text-[#aaa]">Offices</p><button onClick={() => setShowOffice(true)} className="text-lg text-[#777]">+</button></div>{offices.map(item => <button key={item.id} onClick={() => { setOffice(item); setView("today"); }} className={`mb-1 w-full rounded-lg px-3 py-2 text-left text-sm ${office?.id === item.id ? "bg-[#173b32] text-white" : "text-[#666] hover:bg-[#f5f5f2]"}`}>{item.name}</button>)}{!offices.length && <p className="px-3 text-xs leading-5 text-[#999]">Create your first office to get started.</p>}</div><button onClick={signOut} className="mt-10 px-3 text-sm text-[#888]">Sign out</button></aside>
    <section className="min-w-0 flex-1 px-5 py-6 md:px-9 md:py-8"><header className="flex items-center justify-between"><p className="text-sm text-[#888]">Founder workspace</p><span className="text-sm text-[#666]">{profile.full_name}</span></header>
      {!office ? <section className="mx-auto mt-24 max-w-lg text-center"><h1 className="text-4xl font-semibold tracking-[-0.05em]">Create your first office.</h1><p className="mx-auto mt-4 max-w-md text-sm leading-6 text-[#777]">Once created, this office is where you manage its people and attendance.</p><button onClick={() => setShowOffice(true)} className="mt-7 rounded-xl bg-[#173b32] px-5 py-3 text-sm font-semibold text-white">Add office</button></section> : <>
  ...
  {showOffice && <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-5"><form onSubmit={createOffice} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl"><div className="flex items-start justify-between"><div><h2 className="text-xl font-semibold">Add office</h2><p className="mt-1 text-sm text-[#777]">Set the office boundary from your current location.</p></div><button type="button" onClick={() => setShowOffice(false)} className="text-[#888]">×</button></div><div className="mt-6 space-y-3"><input required value={officeName} onChange={e => setOfficeName(e.target.value)} placeholder="Office name" className="w-full rounded-xl border border-black/10 px-4 py-3 text-sm"/><input value={officeAddress} onChange={e => setOfficeAddress(e.target.value)} placeholder="Address" className="w-full rounded-xl border border-black/10 px-4 py-3 text-sm"/><div className="rounded-xl border border-black/10 p-4"><div className="flex items-center justify-between"><span className="text-sm font-medium">Office location</span><button type="button" onClick={getLiveLocation} className="rounded-lg bg-[#173b32] px-3 py-2 text-xs font-semibold text-white">{tracking ? "Finding…" : "Use my live location"}</button></div><p className="mt-2 text-xs text-[#888]">{location ? `Located at ${location.lat.toFixed(5)}, ${location.lng.toFixed(5)}` : "Location required"}</p></div><label className="block text-sm font-medium">Geofence radius<select value={radius} onChange={e => setRadius(Number(e.target.value))} className="mt-2 w-full rounded-xl border border-black/10 px-4 py-3 text-sm"><option value={100}>100 m</option><option value={150}>150 m</option><option value={250}>250 m</option><option value={500}>500 m</option></select></label><button disabled={busy || !location} className="w-full rounded-xl bg-[#173b32] py-3 text-sm font-semibold text-white disabled:opacity-40">{busy ? "Creating…" : "Create office"}</button></div></form></div>}
  {showEmployee && office && <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-5"><form onSubmit={addEmployee} className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl"><div className="flex items-start justify-between"><div><h2 className="text-xl font-semibold">Add employee</h2><p className="mt-1 text-sm text-[#777]">Adding to {office.name}</p></div><button type="button" onClick={() => setShowEmployee(false)} className="text-[#888]">×</button></div><div className="mt-6 space-y-3"><input required value={employeeName} onChange={e => setEmployeeName(e.target.value)} placeholder="Full name" className="w-full rounded-xl border border-black/10 px-4 py-3 text-sm"/><input required type="email" value={employeeEmail} onChange={e => setEmployeeEmail(e.target.value)} placeholder="Work email" className="w-full rounded-xl border border-black/10 px-4 py-3 text-sm"/><input value={employeeId} onChange={e => setEmployeeId(e.target.value)} placeholder="Employee ID (optional)" className="w-full rounded-xl border border-black/10 px-4 py-3 text-sm"/><button disabled={busy} className="w-full rounded-xl bg-[#173b32] py-3 text-sm font-semibold text-white disabled:opacity-40">{busy ? "Adding…" : "Add employee"}</button></div></form></div>}
  </main>;
}
...
function AttendanceRows({ people, attendance }: { people: Profile[]; attendance: Attendance[] }) { return <div className="divide-y divide-black/5">{people.map(person => { const record = attendance.find(item => item.employee_id === person.id); return <div key={person.id} className="flex items-center justify-between px-5 py-4"><div><p className="font-medium">{person.full_name}</p><p className="mt-1 text-xs text-[#888]">{person.employee_id || "No ID"}</p></div><div className="text-right"><p className="text-sm font-medium capitalize">{record?.status || "Absent"}</p><p className="mt-1 text-xs text-[#888]">{record?.check_in_at ? time(record.check_in_at) : "—"}{record?.check_out_at ? ` → ${time(record.check_out_at)}` : ""}</p></div></div>; })}{!people.length && <p className="px-5 py-8 text-center text-sm text-[#888]">No employees in this office yet.</p>}</div>; }
