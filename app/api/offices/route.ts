...
export async function GET(request: Request) {
  ...
  const { data, error } = await ctx.admin
    .from("offices")
    .select("id,name,address,latitude,longitude,geofence_radius_m,geofence_enabled,work_start,work_end,grace_minutes")
...
export async function POST(request: Request) {
  ...
  const { data, error } = await ctx.admin
    ...
    .insert({
      ...
      geofence_enabled: latitude != null && longitude != null,
      archived_at: null,
    })
    .select()
    .single();
...
export async function DELETE(request: Request) {
  ...
  if (!officeId) return NextResponse.json({ error: "Office ID is required" }, { status: 400 });
  ...
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  if (!data) return NextResponse.json({ error: "Office not found or already archived" }, { status: 404 });
  return NextResponse.json({ office: data, archived: true });
}
