...
export async function POST(request: Request) {
  ...
  const authClient=createClient(url,anonKey); const {data:{user},error:userError}=await authClient.auth.getUser(accessToken);
  ...
  const body=await request.json(); const {email,fullName,officeId,department,designation,employeeId}=body;
