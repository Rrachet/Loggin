...
begin
  ...
  if requested_company_id is not null then
    ...
    requested_role := coalesce(nullif(new.raw_user_meta_data->>'role', '')::public.user_role, 'employee'::public.user_role);
  else
...
-- Managers can see attendance for their own company, but employees only see their own records.
...
using (
  ...
  or exists (
    ...
    where viewer.id = auth.uid()
      and viewer.company_id = (select company_id from public.profiles where id = employee_id)
...
-- Employees may not manufacture founder/admin profiles from the browser.
